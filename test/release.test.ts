...
describe('GitHub release contract', () => {
  ...
  it('uses stable platform-specific artifact names', async () => {
    ...
    expect(packageJson.build.extraResources).toContainEqual({
      from: 'build/installer-catalog',
      to: 'installer-catalog'
    })
  ...
  it('uses a one-page NSIS wizard with a read-only catalog and per-user install', async () => {
    const installer = await readFile(
      path.join(projectRoot, 'build', 'installer.nsh'),
      'utf8'
    )
    const labels = await readFile(
      path.join(projectRoot, 'build', 'installer-catalog-labels.nsh'),
      'utf8'
    )
    const sidebar = await readFile(path.join(projectRoot, 'build', 'installer-sidebar.bmp'))
    const logo = await readFile(path.join(projectRoot, 'build', 'installer-logo.bmp'))
    ...
    expect(installer).toContain('StrCpy $DshUserInstDir $INSTDIR')
    expect(installer).toContain('StrCpy $INSTDIR $DshUserInstDir')
    expect(installer).toContain('StrCpy $isForceCurrentInstall "1"')
    expect(installer).toContain('Page custom DshWelcomeCreate DshWelcomeLeave')
    expect(installer).toContain('Page custom DshApplyChosenDirectory')
    expect(installer).toContain('STR:Install')
    expect(installer).toContain('STR:Update')
    expect(installer).toContain('DshDetectExistingInstall')
    expect(installer).toContain('IfFileExists "$INSTDIR\\${APP_FILENAME}.exe"')
    expect(installer).toContain('taskkill.exe')
    expect(installer).toContain('This update replaces the app and bundled plugins:')
    expect(installer).toContain('installer-catalog-labels.nsh')
    expect(installer).toContain('!insertmacro DshFillCatalogList $DshCatalogList')
    expect(installer).toContain('${NSD_SetImage} $0 "$PLUGINSDIR\\installer-logo.bmp" $DshLogoImage')
    expect(installer).toContain('installer-logo.bmp')
    expect(installer).not.toContain('!macro customHeader')
    expect(installer).toContain('$LOCALAPPDATA\\Programs\\${PRODUCT_FILENAME}')
    ...
    expect(installer).toContain('Function .onVerifyInstDir')
    expect(installer).toContain('!macro customInstall')
    expect(installer).toContain('$APPDATA\\dsh-desktop\\harness\\.desktop-catalog-applied')
    expect(installer).toContain('$APPDATA\\dsh-desktop-dev\\harness\\.desktop-catalog-applied')
    ...
    expect(logo.subarray(0, 2).toString('ascii')).toBe('BM')
  })
  ...
  it('clears the leftover session marker during a Windows overwrite install', async () => {
    ...
    // Anchor on the macro name: this bundle also defines customInstallMode, and a
    // loose /customInstall/ would match that macro first.
    const customInstall = installer.match(/!macro customInstall\r?\n[\s\S]*?!macroend/)?.[0]
  ...
  it('keeps builder jobs from attempting implicit tag publishing', async () => {
    ...
    const packageWin = await readFile(path.join(projectRoot, 'scripts', 'package-win.mjs'), 'utf8')
    expect(packageJson.scripts['package:win']).toContain('package-win.mjs')
    expect(packageWin).toContain('--publish')
    expect(packageWin).toContain('never')
    expect(packageWin).toContain("stdio: 'inherit'")
    expect(packageWin).not.toMatch(/\bexecSync\s*\(/)
  ...
  it('packages an isolated development channel from the current workspace', async () => {
    ...
    expect(packageJson.scripts['package:dev:win']).toContain('prepare-installer-catalog.mjs')
    ...
    const packageWin = await readFile(path.join(projectRoot, 'scripts', 'package-win.mjs'), 'utf8')
    expect(packageJson.scripts['package:win']).toContain('package-win.mjs')
    expect(packageWin).toContain('prepare-installer-catalog.mjs')
    expect(packageWin).toContain('verify-target.mjs')
    expect(packageWin).toContain("'verify-target'")
    expect(packageWin).toContain("'build'")
    expect(packageWin).toContain("'prepare-installer-catalog'")
    expect(packageWin).toContain("'electron-builder'")
    expect(packageWin).toContain("stdio: 'inherit'")
    expect(packageWin).toContain('stage:')
