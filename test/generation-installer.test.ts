...
import { packDirectoryAsTgz } from '../scripts/prepare-installer-catalog.mjs'
...
describe('the generation installer', () => {
  ...
  it('forwards Profile build approvals into the isolated pnpm staging workspace', async () => {
    ...
    const result = await installGeneration({
      ...
      runInstall: stubInstall(async (staging) => {
        ...
        expect(await readFile(join(staging, '.npmrc'), 'utf8')).toContain('auto-install-peers=false')
  ...
  it('installs a file: catalog tarball without contacting the npm registry', async () => {
    const home = await freshHome()
    ...
    const packed = packDirectoryAsTgz(pluginDir, home)

    let spawned = 0
    const result = await installGeneration({
      dshHome: home,
      pluginSpec: `file:${packed}`,
      expectedPluginName: 'fixture-sidebar',
      nodeExecutablePath: 'node',
      pnpmEntryPath: 'pnpm',
      offline: true,
      spawnProcess: (...args: unknown[]) => {
        spawned += 1
        throw new Error(`pnpm must not run for file: specs: ${JSON.stringify(args)}`)
      }
    })

    expect(isFilePluginSpec(`file:${packed}`)).toBe(true)
    expect(spawned).toBe(0)
    expect(result.ok).toBe(true)
    expect(result.generation?.pluginName).toBe('fixture-sidebar')
    const installed = JSON.parse(
      await readFile(
        join(result.generation!.directory, 'node_modules', 'fixture-sidebar', 'package.json'),
        'utf8'
      )
    )
  ...
  it('copies missing file: production deps from the Desktop host node_modules, not the registry', async () => {
    const home = await freshHome()
    const hostModules = join(home, 'host-node_modules')
    const tiny = join(hostModules, 'tiny-dep')
    await mkdir(tiny, { recursive: true })
    await writeFile(
      join(tiny, 'package.json'),
      JSON.stringify({ name: 'tiny-dep', version: '1.2.3', main: 'index.js' })
    )
    await writeFile(join(tiny, 'index.js'), 'module.exports = 1\n')
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        version: '2.0.0',
        main: 'index.js',
        dependencies: { 'tiny-dep': '1.2.3' },
        dsh: { bundle: { patch: 'p.yml' } }
      })
    )
    await writeFile(join(pluginDir, 'index.js'), 'module.exports = {}\n')
    await writeFile(join(pluginDir, 'p.yml'), '[]\n')
    const packed = packDirectoryAsTgz(pluginDir, home)

    const result = await installGeneration({
      dshHome: home,
      pluginSpec: `file:${packed}`,
      expectedPluginName: 'needs-tiny',
      nodeExecutablePath: 'node',
      pnpmEntryPath: 'pnpm',
      hostNodeModulesPath: hostModules,
      offline: true,
      spawnProcess: () => {
        throw new Error('registry fetch is forbidden')
      }
    })

    expect(result.ok).toBe(true)
    expect(
      await readFile(join(result.generation!.directory, 'node_modules', 'tiny-dep', 'index.js'), 'utf8')
