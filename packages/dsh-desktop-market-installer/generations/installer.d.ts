...
export interface GenerationInstallOptions {
  ...
  /**
   * `file:` catalog/tarball installs copy missing production deps from here
   * instead of fetching registry.npmjs.org. Typically the Desktop app's
   * `node_modules`.
   */
  hostNodeModulesPath?: string
  /** Refuse registry fetches (`offline=true` / `pnpm add --offline`). */
  offline?: boolean
...
export function isFilePluginSpec(spec: string): boolean

export function filePluginSpecPath(spec: string): string
